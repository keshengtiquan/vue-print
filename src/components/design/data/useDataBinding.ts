...
export function useDataBinding() {
  ...
  /**
   * 取数上下文：把一个元素 + 一条记录变成 `renderTemplate` 要的 `TemplateContext`。
   *
   * ⚠️ **当前没有任何消费方** —— 设计态画布只显示占位符原文（§6.7 / 表格文档 §1.5 约束 2），
   * 三个渲染元素里没有一处调用；面板上的「取数预览」也已按老板要求移除（§11.16）。
   * 它是**渲染层与数据层之间那条接缝**，留给将来的预览 / 导出 / 打印态 ——
   * 那时"把占位符变成值"只该有一个入口，就是这里。
   *
   * `onMissing: "keep"` 是刻意的：取不到值时保留 `{字段}` 原文而不是留白 ——
  ...
  function designContext(el: Element | undefined | null, row?: DataRow): TemplateContext {
    ...
    const target = row ?? binding.sampleOf(ds.id)[0];
    return buildRowContext(target, {
      dataSetName: ds.name,
      fields: ds.fields,
      sys: sys(),
      onMissing: "keep"
